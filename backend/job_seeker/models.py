...
class CareerPreference(models.Model):
  user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='job_seeker_career_preference')
  prefered_job_category = models.OneToOneField(JobCategory, on_delete=models.CASCADE)
  ...
  def __str__(self):
    return f"{self.user.email} preference for {self.prefered_job_category.category}"