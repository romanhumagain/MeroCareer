...
class EditJobPost extends StatefulWidget {
  ...
  const EditJobPost({super.key, required this.id});
...
class _EditJobPostState extends State<EditJobPost> {
  ...
  void _handleJobPostUpdate() async {
    ...
    if (_formKey.currentState?.validate() ?? false) {
      if (_selectedCategoryId == "") {
        ...
        try {
          ...
          final response = await jobProvider.updateJob(widget.id, formData);
          if (response?.statusCode == 200) {
            ...
            showCustomFlushbar(
              context: context,
              message: "Sorry, couldn't update job. please try again !",
  ...
  Future<Map<String, dynamic>> fetchJobDetails() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _jobTitleController.text = responseData['job_title'] ?? '';
        _noOfVacancyController.text =
            responseData['no_of_vacancy'].toString() ?? '';
        _deadlineController.text = responseData['deadline'] ?? '';
        _jobRequirementsController.text = responseData['job_requirement'] ?? '';
        _minSalaryController.text =
            responseData['salary_range']?.split('-')[0] ?? '';
        _maxSalaryController.text =
            responseData['salary_range']?.split('-')[1] ?? '';

        // Set the selected values for drop-downs or selectors
        _selectedDegree = responseData['degree'] ?? '';
        _selectedCategory = responseData['category_name']?.toString() ?? '';
        _selectedCategoryId = responseData['category']?.toString() ?? '';
        _selectedJobType = responseData['job_type'] ?? '';
        _selectedJobLevel = responseData['job_level'] ?? '';
        _experience = responseData['experience'] ?? 0;

        skills.clear();
        skills.addAll(List<String>.from(responseData['skills_display'] ?? []));
                            ...
                            child: Column(
                              children: [
                                    ...
                                    Padding(
                                      ...
                                      child: TextFormField(
                                        ...
                                        decoration: InputDecoration(
                                          ...
                                          hintText:
                                              "Enter the job requirements here",
                                          hintStyle: TextStyle(
                                              color: Colors.grey,
                                ...
                                MyButton(
                                    ...
                                    height: 45,
                                    text: "Update Job Posts",
                                    onTap: () {
                                      _handleJobPostUpdate();
                                    }),
                                SizedBox(
                                  height: 26,
                                ),
